...
class ChessboardCollector:
    ...
    def process_image(self, color_msg, depth_msg):
        if not self.camera_info_received:
            return
    ...
    def run(self):
        # We process the loop at a target rate (e.g., 30 Hz)
        rate = rospy.Rate(30)
        while not rospy.is_shutdown():
            ...
            with self.lock:
                if self.color_msg is not None and self.depth_msg is not None:
                    ...
                    # Clear out the messages so we only process new ones
                    self.color_msg = None
                    self.depth_msg = None
            
            if color_msg is not None and depth_msg is not None:
                self.process_image(color_msg, depth_msg)
            else:
                # Keep GUI responsive even if no new messages are processed
                cv2.waitKey(1)
