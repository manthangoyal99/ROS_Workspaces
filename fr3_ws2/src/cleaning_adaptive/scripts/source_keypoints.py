...
class ScribbleMeshGenerator:
    def __init__(self):
        rospy.init_node('scribble_mesh_generator', anonymous=True)

        # --- CONFIGURATION ---
        self.base_frame = "panda_link0"
        self.camera_frame = "camera_color_optical_frame"
        self.SAVE_DIR = "/home/ravi/fr3_ws/src/cleaning_adaptive/data"
        ...
        # --- ROI INTERFACE ---
        self.roi_points = []
        self.window_name = "Scribble Mesh Generator"
        self.window_initialized = False  # <--- FIX: Flag to initialize on the correct thread
        ...
        rospy.loginfo("Waiting for Camera Info, Color, and Aligned Depth...")

    def mouse_cb(self, event, x, y, flags, param):
        """ Captures mouse clicks to define the ROI polygon """
    ...
    def info_cb(self, msg):
        ...
        rospy.loginfo("Camera model initialized.")
    ...
    def color_cb(self, msg):
        ...
        # <--- FIX: Initialize the window here, on the correct thread --->
        if not self.window_initialized:
            cv2.namedWindow(self.window_name)
            cv2.setMouseCallback(self.window_name, self.mouse_cb)
            self.window_initialized = True
            rospy.loginfo("Ready. Click 4 points around the scribble to define the Region of Interest.")

        # Check if Depth and Camera Info are ready
        ...
        # Draw the points and lines for user feedback
        ...
        # Only process vision if we have a full 4-point polygon
        if len(self.roi_points) == 4:
        ...
        if len(self.roi_points) < 4:
            ...
            cv2.putText(cv_image, "MESH: LOCKED", (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(cv_image, "[s] Save Mesh", (15, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 1)
        else:
            cv2.putText(cv_image, "MESH: NO INK FOUND", (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    ...
    def save_mesh(self):
        labels = [f"Mesh_P{i}" for i in range(len(self.current_mesh_base))]
        labeled_keypoints = [{"label": labels[i], "coords": pt} for i, pt in enumerate(self.current_mesh_base)]
        ...
        filename = os.path.join(self.SAVE_DIR, "cleaning_keypoints.json")
        # filename = os.path.join(self.SAVE_DIR, "target_keypoints.json")

        with open(filename, 'w') as f:
            json.dump(data, f, indent=4)
        
        rospy.loginfo(f"SAVED {len(self.current_mesh_base)} mesh points to {filename}")

if __name__ == '__main__':
    try:
        ScribbleMeshGenerator()
        rospy.spin()
