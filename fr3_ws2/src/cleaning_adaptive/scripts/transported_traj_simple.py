...
# --- CONFIGURATION ---
DATA_DIR ="/home/ravi/fr3_ws/src/cleaning_adaptive/data"  # Or set a specific path like "./data"
SOURCE_KP_FILE = "cleaning_keypoints.json"
TARGET_KP_FILE = "target_keypoints.json"
DEMO_TRAJ_FILE = "cleaning_demo_trajectory.json"
...
def transport_demo():
    ...
    # kernel = C(1.0) * RBF(length_scale=[0.08, 0.08, 0.08]) + WhiteKernel(noise_level=1e-4)
    # kernel = C(1.0, constant_value_bounds='fixed') * RBF(length_scale=0.05, length_scale_bounds='fixed')
    kernel = C(1.0, constant_value_bounds='fixed') * RBF(length_scale=[0.1, 0.1, 0.3], length_scale_bounds='fixed')
    # gp = GPWarper(kernel=kernel, n_restarts_optimizer=5, alpha=1e-3, normalize_y=True)
    gp = GPWarper(kernel=kernel, alpha=1e-3, optimizer=None, normalize_y=False)
    
    # Learn mapping: Affine(Source) -> Target
    gp.fit(S_affine, T)
    print("Optimized Kernel:", gp.gp.kernel_)
    ...
    for i in range(len(X_affine)):
        ...
        # C. Orientation Warp
        # Apply polar decomposition to get the pure rotation from the Jacobian
        R_warp, S_warp = polar(J_gp)
