...
try:
    DATA_DIR = os.path.join(rospack.get_path('TPGPT_two_args'), 'data')
except Exception:
...
def load_json_with_fallback(skill, base_name, fallback_name=None):
    if skill:
        name, ext = os.path.splitext(base_name)
        skill_name = f"{name}_{skill}{ext}"
        path = os.path.join(DATA_DIR, skill_name)
        if os.path.exists(path):
            return load_json(skill_name)
    primary_fallback = fallback_name if fallback_name else base_name
    return load_json(primary_fallback)

def get_keypoint_array(json_data):
    """Extracts coordinate array from keypoint dictionary or list"""
    ...
    if 'keypoints' in json_data:
        kp_data = json_data['keypoints']
        # If there is a specific order reference, follow it
        if 'order_reference' in json_data and isinstance(kp_data, list) and len(kp_data) > 0 and isinstance(kp_data[0], dict):
            kp_map = {kp['label']: kp['coords'] for kp in kp_data}
            try:
                return np.array([kp_map[label] for label in json_data['order_reference']])
            except KeyError:
                pass
        
        # Parse list of points (either dicts with 'coords' or raw lists)
        if isinstance(kp_data, list) and len(kp_data) > 0:
            if isinstance(kp_data[0], dict) and 'coords' in kp_data[0]:
...
def main():
    ...
    parser.add_argument('--skill', type=str, default=None, help='Skill name (e.g., pick, push)')
    args, unknown = parser.parse_known_args()
    skill = args.skill

    # 1. Load All Data with appropriate fallbacks
    demo_data = load_json_with_fallback(skill, 'demo_trajectory.json')
    warp_data = load_json_with_fallback(skill, 'warped_trajectory.json')
    affine_data = load_json_with_fallback(skill, 'affine_trajectory.json')
    source_keypoints = load_json_with_fallback(skill, 'keypoints.json', 'source_keypoints.json')
    target_keypoints = load_json_with_fallback(skill, 'target_keypoints.json')
    source_affine_keypoints = load_json_with_fallback(skill, 'source_affine_keypoints.json')
    source_final_keypoints = load_json_with_fallback(skill, 'source_final_keypoints.json')

    if demo_data is None or warp_data is None or affine_data is None:
        print(f"Error: Missing trajectory data for skill '{skill or 'generic'}'. Cannot visualize.")
        return
    ...
    ax.plot(warp_pos[:,0], warp_pos[:,1], warp_pos[:,2], 
            label='Non-Affine Warped Policy', color='green', linewidth=2, linestyle='--')
    ...
    # Target Keypoints (Magenta) - Correspond to Orange/Green Lines
    if T_kp.shape[0] > 0:
    ...
    # 4. Setup Plot 2 (Deformable Warping detail)
    fig = plt.figure(figsize=(14, 10))
    ax = fig.add_subplot(111, projection='3d')

    # --- PLOT TRAJECTORIES ---
    ax.plot(affine_pos[:,0], affine_pos[:,1], affine_pos[:,2], 
            label='Affine Warped Policy', color='blue', linewidth=2, alpha=0.7)
    
    ax.plot(warp_pos[:,0], warp_pos[:,1], warp_pos[:,2], 
            label='Non-Affine Warped Policy', color='darkorange', linewidth=2)
